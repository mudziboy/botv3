...
# =======================================================
# PART 1: FONDASI, WARNA, OTORISASI GITHUB
# =======================================================

# Color definitions (Diperkaya)
green="\e[38;5;82m"
red="\e[38;5;196m"
neutral="\e[0m"
orange="\e[38;5;208m"
...
print_rainbow() {
    local text="$1"
    local length=${#text}
    local start_color=(255 100 0)
    local end_color=(0 200 255)
    for ((i = 0; i < length; i++)); do
        local r=$(( (start_color[0] * (length - i) + end_color[0] * i) / length ))
        local g=$(( (start_color[1] * (length - i) + end_color[1] * i) / length ))
        local b=$(( (start_color[2] * (length - i) + end_color[2] * i) / length ))
...
print_rainbow "    └───────────────────────────────────────────────┘"
echo -e "${blue}    ────────────────────────────────────────────────${neutral}"
echo -e "${yellow}       Sistem Otorisasi IP Otomatis by Rahmarie${neutral}"
echo -e "${blue}    ────────────────────────────────────────────────${neutral}"
...
REPO_NAME="regist"
FILE_PATH="izin"
IZIN_URL="https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/${FILE_PATH}"
IZIN_FILE="/tmp/izin_whitelist.txt" # Simpan di /tmp

echo -e "${orange}Mengecek otorisasi IP VPS ($ip) dengan GitHub...${neutral}"
loading_bar 50 # Menambah bar loading untuk sinkronisasi

# 1. Unduh file izin terbaru
if wget -q -O $IZIN_FILE $IZIN_URL; then
    echo -e "${green}✅ File izin berhasil disinkronkan dari GitHub.${neutral}"
else
    echo -e "${red}❌ Gagal mengunduh file izin dari GitHub. Cek URL atau koneksi.${neutral}"
    echo -e "${red}   INSTALASI DIBATALKAN. Akses skrip utama GAGAL. ${neutral}"
    exit 1
fi

# 2. Lakukan Pengecekan Izin ke file yang baru diunduh
if ! grep -q "^$ip$" $IZIN_FILE; then 
    echo -e "${red}────────────────────────────────────────────────${neutral}"
    echo -e "${red}❌ Otorisasi GAGAL! IP VPS ($ip) tidak terdaftar di file izin.${neutral}"
    echo -e "${yellow}   Akses skrip ditolak. Daftarkan IP melalui Bot IP Whitelist!${neutral}"
    echo -e "${red}────────────────────────────────────────────────${neutral}"
    rm -f $IZIN_FILE # Bersihkan file sementara
    exit 1
fi

echo -e "${green}✅ Otorisasi BERHASIL. IP ($ip) diizinkan.${neutral}"
rm -f $IZIN_FILE
...
# Ganti semua URL mudziboy/mdzitr ke nama github dan repo kamu
...
MENU_URL="https://github.com/mudziboy/mdzitr/raw/main/project/project.zip" 
...
if wget -O menu.zip "$MENU_URL" >/dev/null 2>&1; then
    : # Success, proceed
else
    echo -e "${red}❌ Gagal mengunduh file menu project.zip.${neutral}"
    exit 1
fi
...
wget -q -O /root/.bot/bot.zip https://raw.githubusercontent.com/mudziboy/mdzitro/raw/main/bot.zip
